...
区别就在于，多加了一层 `EasyWin32::isAnyWindow()` 的判断，用以检测是否还存在任何窗口，如果按下按键，或者所有窗口都被销毁，都会结束程序。在原生的 EasyX 中，由于只可能出现一个窗口，所以一旦窗口关闭，EasyX 将默认结束程序，则不再需要这层判断。

这步判断是很关键而又容易被忽略的，在后面的例子中还会讲到，如果不进行窗口是否还存在的判断的话，就会导致进程在后台残留，除非您使用 `EasyWin32::init_end()`，这在后文会提及。
